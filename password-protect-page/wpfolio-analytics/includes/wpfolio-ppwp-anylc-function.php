 ...
 * @since 1.0
 */
function wpfolio_ppwp_anylc_clean( $var ) {
	if ( is_array( $var ) ) {
		return array_map( 'wpfolio_ppwp_anylc_clean', $var );
	} else {
 ...
 * @since 1.0
 */
function wpfolio_ppwp_anylc_is_multi_arr( $arr ) {
    rsort( $arr );
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_site_uid() {

	$site_uid = get_option( 'wpfolio_ppwp_anylc_site_uid' );
	...
	if( empty( $site_uid ) ) {
		...
		update_option( 'wpfolio_ppwp_anylc_site_uid', $site_uid );		
	}
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_optin_data( $anylc_pdt = false, $return_url = '' ) {
	...
	$optin_data = array(
						...
						'sdk_version'		=> WPFOLIO_PPWP_ANYLC_VERSION,
						...
						'ip_address'		=> wpfolio_ppwp_anylc_get_ip_address(),
						'site_uid'			=> wpfolio_ppwp_anylc_site_uid(),
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_get_ip_address() {
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_get_option( $key = '' ) {
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_update_option( $key = '', $data = array() ) {

	$opt_in_data = array();

	if( !empty( $key ) ) {
		$opt_in_data = wpfolio_ppwp_anylc_get_option( $key );
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_optin_url( $module_data = '', $optin_status = null ) {
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_optout_url( $module_data = '', $optin_status = null, $redirect_url = false ) {
 ...
 * @since 1.0.0
 */
function wpfolio_ppwp_anylc_pdt_url( $module_data = '', $type = false ) {
		...
		switch ( $type ) {
			case 'promotion':
				...
				if( !empty( $promotion ) ) {
					...
					$redirect_url 	= add_query_arg( array( 'message' => 'wpfolio_ppwp_anylc_promotion', 'wpfolio_ppwp_anylc_pdt' => $module_data['slug'], 'wpfolio_ppwp_anylc_promo_pdt' => $promotion ), $redirect_url );
				}
				break;
